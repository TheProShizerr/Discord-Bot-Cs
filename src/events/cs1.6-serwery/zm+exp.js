...
const { ZombieModInfo } = require('../../baza')

const channelId = '1175384545026129950'
...
const updateServerInfo = async client => {
	...
	try {
		...
		const newPlayerCount = new ZombieModInfo({
			playerCountTimestamp: Date.now(),
			playerCount: serverInfo.players.length,
		})
		await newPlayerCount.save()

		let lastMapDoc = await ZombieModInfo.findOne({ lastMapName: { $exists: true } })
		if (!lastMapDoc) {
			lastMapDoc = new ZombieModInfo({ lastMapName: serverInfo.map })
		...
		if (lastMapDoc.lastMapName !== serverInfo.map) {
			...
			let mapDoc = await ZombieModInfo.findOne({ mapName: serverInfo.map })
			if (mapDoc) {
				mapDoc.mapCount++
				await mapDoc.save()
			} else {
				mapDoc = new ZombieModInfo({ mapName: serverInfo.map, mapCount: 1 })
				await mapDoc.save()
			}
		}
		...
		const ignoredPlayers = [
			'Forum - CSowicze.pl',
			'discord.gg/YxpAaRy4AH',
			'IP: 51.83.166.59:27015',
			'Zakupy na /sklepsms',
		]

		const rows = await ZombieModInfo.find({ highlightedPlayerName: { $exists: true } })
		...
		const mapDoc = await ZombieModInfo.findOne({ mapName: serverInfo.map })
		...
		const row = await ZombieModInfo.findOne({ channelId: { $exists: true } })
		if (row) {
		...
		if (serverInfoMessage) {
			...
			const newServerInfoMessage = new ZombieModInfo({
				channelId: channelId,
