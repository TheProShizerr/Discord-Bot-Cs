...
const { PbModel } = require('../../baza')

const channelId = '1175384519067570197'
...
const updateServerInfo = async client => {
	...
	try {
		...
		const newPlayerCount = new PbModel({
			playerCountTimestamp: Date.now(),
			playerCount: serverInfo.players.length,
		})
		await newPlayerCount.save()

		let lastMapDoc = await PbModel.findOne({ lastMapName: { $exists: true } })
		if (!lastMapDoc) {
			lastMapDoc = new PbModel({ lastMapName: serverInfo.map })
		...
		if (lastMapDoc.lastMapName !== serverInfo.map) {
			...
			let mapDoc = await PbModel.findOne({ mapName: serverInfo.map })
			if (mapDoc) {
				mapDoc.mapCount++
				await mapDoc.save()
			} else {
				mapDoc = new PbModel({ mapName: serverInfo.map, mapCount: 1 })
				await mapDoc.save()
			}
		}

		const ignoredPlayers = ['HLTV: 1s1k najlepsze serwery PL']

		const rows = await PbModel.find({ highlightedPlayerName: { $exists: true } })
		...
		const mapDoc = await PbModel.findOne({ mapName: serverInfo.map })
		...
		const row = await PbModel.findOne({ channelId: { $exists: true } })
		if (row) {
		...
		if (serverInfoMessage) {
			...
			const newServerInfoMessage = new PbModel({
				channelId: channelId,
