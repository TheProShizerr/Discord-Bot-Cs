...
const { FFaModel } = require('../../baza')

const channelId = '1175384459982413824'
...
const updateServerInfo = async client => {
	...
	try {
		...
		const newPlayerCount = new FFaModel({
			playerCountTimestamp: Date.now(),
			playerCount: serverInfo.players.length,
		})
		await newPlayerCount.save()

		let lastMapDoc = await FFaModel.findOne({ lastMapName: { $exists: true } })
		if (!lastMapDoc) {
			lastMapDoc = new FFaModel({ lastMapName: serverInfo.map })
		...
		if (lastMapDoc.lastMapName !== serverInfo.map) {
			...
			let mapDoc = await FFaModel.findOne({ mapName: serverInfo.map })
			if (mapDoc) {
				mapDoc.mapCount++
				await mapDoc.save()
			} else {
				mapDoc = new FFaModel({ mapName: serverInfo.map, mapCount: 1 })
				await mapDoc.save()
			}
		}

		const ignoredPlayers = ['FFA+４FUN TV @hostplay.pl']

		const rows = await FFaModel.find({ highlightedPlayerName: { $exists: true } })
		...
		const mapDoc = await FFaModel.findOne({ mapName: serverInfo.map })
		...
		const row = await FFaModel.findOne({ channelId: { $exists: true } })
		if (row) {
		...
		if (serverInfoMessage) {
			...
			const newServerInfoMessage = new FFaModel({
				channelId: channelId,
