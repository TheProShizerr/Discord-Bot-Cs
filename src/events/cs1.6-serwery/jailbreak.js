...
const { JailbreakModel } = require('../../baza')

const channelId = '1175384482065424405'
...
const updateServerInfo = async client => {
	...
	try {
		...
		const newPlayerCount = new JailbreakModel({
			playerCountTimestamp: Date.now(),
			playerCount: serverInfo.players.length,
		})
		await newPlayerCount.save()

		let lastMapDoc = await JailbreakModel.findOne({ lastMapName: { $exists: true } })
		if (!lastMapDoc) {
			lastMapDoc = new JailbreakModel({ lastMapName: serverInfo.map })
		...
		if (lastMapDoc.lastMapName !== serverInfo.map) {
			...
			let mapDoc = await JailbreakModel.findOne({ mapName: serverInfo.map })
			if (mapDoc) {
				mapDoc.mapCount++
				await mapDoc.save()
			} else {
				mapDoc = new JailbreakModel({ mapName: serverInfo.map, mapCount: 1 })
				await mapDoc.save()
			}
		}

		const ignoredPlayers = ['HLTV JB']

		const rows = await JailbreakModel.find({ highlightedPlayerName: { $exists: true } })
		...
		const mapDoc = await JailbreakModel.findOne({ mapName: serverInfo.map })
		...
		const row = await JailbreakModel.findOne({ channelId: { $exists: true } })
		if (row) {
		...
		if (serverInfoMessage) {
			...
			const newServerInfoMessage = new JailbreakModel({
				channelId: channelId,
