...
const { codModModel } = require('../../baza')

const channelId = '1175384408908369930'
...
const updateServerInfo = async client => {
	...
	try {
		...
		const newPlayerCount = new codModModel({
			playerCountTimestamp: Date.now(),
			playerCount: serverInfo.players.length,
		})
		await newPlayerCount.save()

		let lastMapDoc = await codModModel.findOne({ lastMapName: { $exists: true } })
		if (!lastMapDoc) {
			lastMapDoc = new codModModel({ lastMapName: serverInfo.map })
		...
		if (lastMapDoc.lastMapName !== serverInfo.map) {
			...
			let mapDoc = await codModModel.findOne({ mapName: serverInfo.map })
			if (mapDoc) {
				mapDoc.mapCount++
				await mapDoc.save()
			} else {
				mapDoc = new codModModel({ mapName: serverInfo.map, mapCount: 1 })
				await mapDoc.save()
			}
		}

		const ignoredPlayers = ['SKLEP SMS /sklepsms']

		const rows = await codModModel.find({ highlightedPlayerName: { $exists: true } })
		...
		const mapDoc = await codModModel.findOne({ mapName: serverInfo.map })
		...
		const row = await codModModel.findOne({ channelId: { $exists: true } })
		if (row) {
		...
		if (serverInfoMessage) {
			...
			const newServerInfoMessage = new codModModel({
				channelId: channelId,
