...
const { dd2Model } = require('../../baza')

const channelId = '1175384440936087552'
...
const updateServerInfo = async client => {
	...
	try {
		...
		const newPlayerCount = new dd2Model({
			playerCountTimestamp: Date.now(),
			playerCount: serverInfo.players.length,
		})
		await newPlayerCount.save()

		let lastMapDoc = await dd2Model.findOne({ lastMapName: { $exists: true } })
		if (!lastMapDoc) {
			lastMapDoc = new dd2Model({ lastMapName: serverInfo.map })
		...
		if (lastMapDoc.lastMapName !== serverInfo.map) {
			...
			let mapDoc = await dd2Model.findOne({ mapName: serverInfo.map })
			if (mapDoc) {
				mapDoc.mapCount++
				await mapDoc.save()
			} else {
				mapDoc = new dd2Model({ mapName: serverInfo.map, mapCount: 1 })
				await mapDoc.save()
			}
		}

		const ignoredPlayers = ['HLTV-aaa@pukawka.pl']

		const rows = await dd2Model.find({ highlightedPlayerName: { $exists: true } })
		...
		const mapDoc = await dd2Model.findOne({ mapName: serverInfo.map })
		...
		const row = await dd2Model.findOne({ channelId: { $exists: true } })
		if (row) {
		...
		if (serverInfoMessage) {
			...
			const newServerInfoMessage = new dd2Model({
				channelId: channelId,
