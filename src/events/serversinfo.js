...
const db = new Database('./serversinfo.db');
...
module.exports = {
  ...
  execute: async (client) => {
      const channelId = '1121406564176236584';
          ...
          if (embedMessage) {
              ...
              if (row) {
                  ...
                  if (embedMessage) {
                      ...
                      db.prepare("UPDATE serverInfoMessage SET messageId = ? WHERE channelId = ?").run(embedMessage.id, channelId);
                  }
              } else {
                  embedMessage = await channel.send({ embeds: [embed] });
                  db.prepare("INSERT INTO serverInfoMessage (channelId, messageId) VALUES (?, ?)").run(channelId, embedMessage.id);
              }
          }
      };
