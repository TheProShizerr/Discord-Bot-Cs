    ...
    run: async (client, message, args) => {
        try {
            ...
            if (ownerRows.length > 0) {
                fields.push({ name: 'Właściciele', value: ownerRows.map(row => `${row.name} (${row.code})`).join(", ") });
            }
            if (leftColumn.length > 0) {
                fields.push({ name: '\u200B', value: leftColumn.map((row, index) => `${index + 1}. ${row.name} (${row.code})`).join("\n"), inline: true });
            }
            if (rightColumn.length > 0) {
                fields.push({ name: '\u200B', value: rightColumn.map((row, index) => `${index + half + 1}. ${row.name} (${row.code})`).join("\n"), inline: true });
            }
            if (maxTimestamp) {
